/*
The following must be done in the opencv-lane-vehicle-track-master folder, immediatly
after starting the terminal:
	Set Display out:
		export DISPLAY=:0
	Disable video feed security policies:
...
enum{
    ...
    BW_TRESHOLD = 10,		  // greyscale response strength to recognize for 'WHITE'
    BORDERX = 10,			  // px, skip this much from left & right borders
	MAX_RESPONSE_DIST = 100,	  // maximum number of pixels the calculated lane can correct per period. 
...
void processLanes(CvSeq* lines, IplImage* edges, IplImage* temp_frame) {
	...
	for	(int i=0; i<right.size(); i++) {
		//cvLine(temp_frame, right[i].p0, right[i].p1, CV_RGB(255, 0, 0), 2);
	} //rigtht lanes

	for	(int i=0; i<left.size(); i++) {
		//cvLine(temp_frame, left[i].p0, left[i].p1, CV_RGB(0, 0, 255), 2);
...
int main(void)
{
...
#ifdef USE_VIDEO
	CvCapture *input_video = cvCreateFileCapture("bin/A.mp4");
#else
	...
	while(key_pressed != 27) {
		...
		// merge cropped detected lanes into one large image:
		cv::Mat outz;
		std::vector<cv::Mat> array_to_merge;
		cv::Mat small = cv::cvarrToMat(temp_frame);
		cv::Mat big  = cv::cvarrToMat(frame);
		small.copyTo(big(cv::Rect(0,small.rows,small.cols, small.rows)));
